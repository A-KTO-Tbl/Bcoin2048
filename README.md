...
# 🟣 Bcoin2048 AutoBot /// HPV 🟣

| **Возможности**                                                    | **Поддерживается**  |
|----------------------------------------------------------------|:---------------:|
| **Многофункциональный** — *выполняет все те же действия, что вы делали бы руками:*<br>**—** *Получение кол-ва доступных игр и запуск их прохождения*;<br>**—** *Ожидание от 6 до 7 часов*;<br>**—** *Повторение действий через 6-7 часов*. |✅|
...
| **Защита сибилла (ботовода)** — *в мире, где проекты вроде Bcoin2048 стремятся блокировать всех ботоводов, я позаботился о вашей безопасности. Реализованный с использованием всей мощи Python, скрипт разработан таким образом, чтобы минимизировать риск блокировки фермы аккаунтов. Каждый аккаунт после инициализации получает уникальный User-Agent, делая его трудным для идентификации как бота. Дополнительно, скрипт случайным образом варьирует время выполнения каждого действия, чтобы симулировать натуральное поведение. Каждый свайп в любую сторону реализован случайным образом. И, конечно, скрипт поддерживает прокси для максимальной анонимности и защиты. Вы можете не использовать их, но настоятельно рекомендую это для вашей безопасности* |✅|
...
- **Linux**
  ...
  - ```
    ...
    ```
  - ```
    git clone https://github.com/A-KTO-Tbl/Bcoin2048
    ```
  - ```
    pip3 install -r Bcoin2048\Core\Tools\requirements.txt
    ```
- **Windows**
  ...
  - Запустится CMD в текущей директории. Далее просто вводим следующие команды:
    - ```
      ...
      ```
    - ```
      pip install -r Bcoin2048\Core\Tools\requirements.txt
      ```
- **Android**
